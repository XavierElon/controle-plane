use ::time::OffsetDateTime; // Corrected import
use anyhow::Result;
use models::*;
use reqwest::Client;
...
async fn run_task(client: &Client, controller: &str, a: Assignment) -> Result<()> {
    let cmd = if a.command.is_empty() { "echo".to_string() } else { a.command[0].clone() };
    ...
    let start = TaskStatusUpdate {
        task_id: a.task_id,
        // Used OffsetDateTime directly since it's now in scope
        state: TaskState::Running { node_id: Uuid::nil(), started_at: OffsetDateTime::now_utc() },
    };
    ...
    let end = if status.success() {
        TaskStatusUpdate {
            task_id: a.task_id,
            state: TaskState::Succeeded { finished_at: OffsetDateTime::now_utc() },
        }
    } else {
        TaskStatusUpdate {
            task_id: a.task_id,
            // Added a comma after finished_at
            state: TaskState::Failed { finished_at: OffsetDateTime::now_utc(), attempts: 1, last_error: format!("exit: {}", status) },
        }
    };
